...
static int bq25790_set_prechrg_vbat(struct bq25790_device *bq, int pre_vbat_lvl)
{
	int reg_val;

	dev_notice(bq->dev, "%s:%d", __func__, pre_vbat_lvl);
	reg_val = pre_vbat_lvl << BQ25790_PRECHRG_VOLT_SHIFT;

	return regmap_update_bits(bq->regmap, BQ25790_PRECHRG_CTRL,
				  BQ25790_PRECHRG_VOLT_MASK, reg_val);
}
			     ...
			     struct bq25790_state *state)
{
	...
	state->hiz_en = !!(chrg_ctrl_0 & BQ25790_HIZ_EN);
	
	state->chrg_en = !!(chrg_ctrl_0 & BQ25790_CHRG_EN);
	if (state->chrg_en)
...
static int bq25790_set_hiz_en(struct bq25790_device *bq, bool hiz_en)
{
	int reg_val;

	dev_notice(bq->dev, "%s:%d", __func__, hiz_en);
	reg_val = hiz_en ? BQ25790_HIZ_EN : 0;

	return regmap_update_bits(bq->regmap, BQ25790_CHRG_CTRL_0,
				  BQ25790_HIZ_EN, reg_val);
}

static int bq25790_set_vac_ovp(struct bq25790_device *bq, int vac_ovp_lvl)
{
	int reg_val;

	dev_notice(bq->dev, "%s:%d", __func__, vac_ovp_lvl);
	reg_val = vac_ovp_lvl << BQ25790_VAC_OVP_SHIFT;

	return regmap_update_bits(bq->regmap, BQ25790_CHRG_CTRL_1,
				  BQ25790_VAC_OVP_MASK, reg_val);
}

static int bq25790_set_recharge_volt(struct bq25790_device *bq, int recharge_volt)
{
	int reg_val;
	dev_notice(bq->dev, "%s:%d", __func__, recharge_volt);
	reg_val = (recharge_volt - BQ25790_VRECHRG_OFFSET_mV) / BQ25790_VRECHRG_STEP_mV;
	
	return regmap_update_bits(bq->regmap, BQ25790_RECHRG_CTRL,
				  BQ25790_VRECHARGE_MASK, reg_val);
}
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		ret = bq25790_set_hiz_en(bq, !val->intval);
		break;
...
static int bq25790_hw_init(struct bq25790_device *bq)
{
	...
	ret = bq25790_set_vac_ovp(bq, 0x01);//22V
	if (ret)
		goto err_out;

	ret = bq25790_set_prechrg_vbat(bq, 0x02);//66.7%
	if (ret)
		goto err_out;

	ret = bq25790_set_recharge_volt(bq, 300);
	if (ret)
		goto err_out;

	dev_notice(bq->dev, "ichrg_curr:%d prechrg_curr:%d chrg_vol:%d"
