...
#define SWITCH_CHARGER_NAME "battery"
#define CP_STANDALONE_CHARGER_NAME "bq2597x-standalone"
#define CP_PRIMARY_CHARGER_NAME "bq2597x-master"
#define CP_SECONDARY_CHARGER_NAME "bq2597x-slave"
...
#define ADAPTER_VOLT_UPPER 10000
#define ADAPTER_VOLT_LOWER 6000
#define ADAPTER_VOLT_UPPER_BYP 100000
#define ADAPTER_VOLT_LOWER_BYP 0
#define ADAPTER_VOLT_FIXED1 5000
#define ADAPTER_VOLT_FIXED2 1
#define ADAPTER_VOLT_FIXED3 0
...
#define CP_STANDALONE_CHARGER_NAME "bq25980-standalone"
#define CP_PRIMARY_CHARGER_NAME "bq25980-master"
#define CP_SECONDARY_CHARGER_NAME "bq25980-slave"
...
#define ADAPTER_VOLT_FIXED1 9000
#define ADAPTER_VOLT_FIXED2 12000
#define ADAPTER_VOLT_FIXED3 5000
#define VBAT_GAP_FOR_CP 300

#endif

#ifdef DUAL_CP
#define BAT_CURR_LOOP_LMT	10000
#define STEP1_CURRENT	BAT_CURR_LOOP_LMT
#define STEP2_CURRENT	8000
#define STEP3_CURRENT	6000
#else
...
#define ADAPTER_CURRENT_TOLE_MIN 300
...
static struct pdpm_config pm_config = {
	...
	.adapter_current_tole	= ADAPTER_CURRENT_TOLE_MIN,
};
...
static void usbpd_check_cp_psy(struct usbpd_pm *pdpm)
{
	...
	if (!pdpm->cp_psy) {
#ifdef DUAL_CP
		pdpm->cp_psy = power_supply_get_by_name(CP_PRIMARY_CHARGER_NAME);
#else
		pdpm->cp_psy = power_supply_get_by_name(CP_STANDALONE_CHARGER_NAME);
#endif
		if (!pdpm->cp_psy)
...
static int usbpd_pm_check_cp_enabled(struct usbpd_pm *pdpm)
{
	...
	//pr_notice("pdpm->cp.charge_enabled:%d ret:%d\n", pdpm->cp.charge_enabled, ret);
	return ret;
}
...
{
	if (!pdpm->cp_sec_psy) {
		pdpm->cp_sec_psy = power_supply_get_by_name(CP_SECONDARY_CHARGER_NAME);
		if (!pdpm->cp_sec_psy)
...
static void usbpd_pm_update_cp_sec_status(struct usbpd_pm *pdpm)
{
	...
	ret = power_supply_get_property(pdpm->cp_sec_psy,
			POWER_SUPPLY_PROP_CURRENT_NOW, &val);
	if (!ret)
		pdpm->cp_sec.ibus_curr = val.intval/1000;
...
static int usbpd_pm_check_cp_sec_enabled(struct usbpd_pm *pdpm)
{
	...
	//pr_notice("pdpm->cp_sec.charge_enable:%d ret:%d\n", pdpm->cp_sec.charge_enabled, ret);
	return ret;
}

static int usbpd_pm_enable_cp_sec_adc(struct usbpd_pm *pdpm, bool enable)
{
	int ret;
	union power_supply_propval val = {0,};
...
static int usbpd_pm_enable_cp_sec_bypass(struct usbpd_pm *pdpm, bool enable)
{
	int ret;
	union power_supply_propval val = {0,};
...
static int usbpd_pm_check_sw_limited(struct usbpd_pm *pdpm)
{
	...
	if (!ret)
	{
		if (val.intval == pm_config.sw_keep_current)
...
static void usbpd_pm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	//msleep(300);//Waiting for adapter discover identity to get full current.
	...
	if (pdpm->adapter_apdo_type > PD_PM_APDO_NONE) {
		...
		ret = usbpd_get_source_pdp_rating(pdpm->pd, &pdp_rating);
		if (ret) {
			pr_err("Failed to get source pdp rating.\n");
		}
		else {
			if (pdp_rating) {
				pdpm->apdo_max_curr = min(pdpm->apdo_max_curr, (pdp_rating*1000)/(pdpm->apdo_max_volt/1000));
			}
			pr_notice("pdp_rating:%d pps_power_limited:%d apdo_max_curr:%d", pdp_rating, pdpm->pps_power_limited, pdpm->apdo_max_curr);
		}
		pm_config.adapter_current_tole = max(pdpm->apdo_max_curr/10, ADAPTER_CURRENT_TOLE_MIN);
...
#ifdef DUAL_CP
			usbpd_pm_enable_cp_sec_bypass(pdpm, false);
#endif
#endif
		}
#ifdef BATTERY_2S
...
#ifdef DUAL_CP
			usbpd_pm_enable_cp_sec_bypass(pdpm, true);
#endif
		}
#endif
...
static int usbpd_pm_fc3_charge_algo(struct usbpd_pm *pdpm)
{
		...
		if (g_fcc_val == STEP1_CURRENT) {
			if (pdpm->cp.vbat_volt > (ADAPTER_CC_TO_CV_VOLTAGE - 2*VBAT_REGU_HYS)) {
				if (adapter_CV_timer++ >= STEP_TIMEOUT) {
					//adapter CV mode
					adapter_CV_timer = 0;
					ibus_limit = pm_config.bus_curr_lp_lmt - pm_config.adapter_current_tole;
				}
			...
			ibat_limit = pm_config.bat_curr_lp_lmt + pm_config.adapter_current_tole*2;//not work, just in case
		}else {
			ibus_limit = pm_config.bus_curr_lp_lmt - pm_config.adapter_current_tole;
	...
	pr_notice("steps:%d step_vbat:%d step_ibat:%d step_ibus:%d ibus_limit:%d ibat_limit:%d "
			"vbat:%d ibat:%d vbus:%d ibus1:%d ibus2:%d\n",
			steps, step_vbat, step_ibat, step_ibus, ibus_limit, ibat_limit,
			pdpm->cp.vbat_volt, pdpm->cp.ibat_curr, pdpm->cp.vbus_volt,
			pdpm->cp.ibus_curr, pdpm->cp_sec.ibus_curr);
...
#ifdef DUAL_CP
			|| (!pdpm->cp_sec.charge_enabled && !pdpm->cp_sec_stopped)
#endif
			) {
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	int thermal_level = 0;
	int gap = 0;
	static int dual_switch_deglitch = 0;
	...
	case PD_PM_STATE_FC3_TUNE:
		...
		if (pdpm->cp_sec.charge_enabled
			...
			if (dual_switch_deglitch++ >=5) {
				pr_notice("second cp is disabled due to one of ibus < 750mA\n");
				usbpd_pm_enable_cp_sec(pdpm, false);
				usbpd_pm_check_cp_sec_enabled(pdpm);
				pdpm->cp_sec_stopped = true;
				dual_switch_deglitch = 0;
			}
		} else if (!pdpm->cp_sec.charge_enabled
				&& (pdpm->cp.ibus_curr > 1700)) {
			if (dual_switch_deglitch++ >=5) {
				pr_notice("second cp is enabled due to ibus > 1700mA\n");
...
#ifdef DUAL_CP
			usbpd_pm_enable_cp_sec_adc(pdpm, false);
#endif
		}
...
#ifdef DUAL_CP
			usbpd_pm_enable_cp_sec_adc(pdpm, true);
#endif
			msleep(100);
...
#ifdef DUAL_CP
	usbpd_pm_enable_cp_sec_adc(__pdpm, false);
#endif
}
